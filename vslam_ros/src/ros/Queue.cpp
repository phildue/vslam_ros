...
namespace vslam_ros{
        ...
        sensor_msgs::msg::Image::ConstPtr Queue::popClosestImg(std::uint64_t t)
        {
                ...
                if (t <= 0)
                {
                        ...
                        {
                                throw std::runtime_error("Did not find image which is close enough to: " + std::to_string(t) + " closest: " + std::to_string(minDiff));
        ...
        sensor_msgs::msg::Image::ConstPtr Queue::popClosestDepth(std::uint64_t t)
        {
                ...
                if (t <= 0)
                {
                        ...
                        if (minDiff > _maxDiff)
                        {
                                throw std::runtime_error("Did not find depth which is close enough to: " + std::to_string(t) + " closest: " + std::to_string(minDiff));
