...
from sensor_msgs.msg import Imu, PointField, NavSatFix, Image, CameraInfo
...
def save_imu_data(bag, kitti, imu_frame_id, topic):
    ...
    for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
        ...
        imu.header.stamp = Time(sec=int(timestamp // 10**9), nanosec=int(timestamp % 10**9))
...
def save_dynamic_tf(bag, kitti, kitti_type, initial_time):
    ...
    if kitti_type.find("raw") != -1:
        for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
            ...
            tf_oxts_transform.header.stamp = Time(sec=int(timestamp // 10**9), nanosec=int(timestamp % 10**9))
            tf_oxts_transform.header.frame_id = 'world'
            tf_oxts_transform.child_frame_id = 'base_link'

            transform = (oxts.T_w_imu)
            t = transform[0:3, 3]
            q = R.from_matrix(transform).as_quat()
            oxts_tf = Transform()
            ...
            bag.write('/tf', serialize_message(tf_oxts_msg), tf_oxts_transform.header.stamp)

    elif kitti_type.find("odom") != -1:
        timestamps = map(lambda x: initial_time + x, kitti.timestamps)

        for timestamp, tf_matrix in zip(timestamps, kitti.poses):
            tf_msg = TFMessage()
            tf_stamped = TransformStamped()
            tf_stamped.header.stamp = Time(sec=timestamp.seconds, nanosec=timestamp.microseconds*1000)
            tf_stamped.header.frame_id = 'world'
            tf_stamped.child_frame_id = 'camera_left'
            
            t = tf_matrix[0:3, 3]
            q = R.from_matrix(tf_matrix[:3,:3]).as_quat()
            transform = Transform()
            ...
            bag.write('/tf', serialize_message(tf_msg), int(timestamp.total_seconds()*10**6))
...
def save_camera_data(bag, kitti_type, kitti, util, bridge, camera, camera_frame_id, topic, initial_time):
    ...
    if kitti_type.find("raw") != -1:
        ...
        image_filenames = sorted(os.listdir(image_path))
        image_datetimes = map(lambda x: initial_time + x, kitti.timestamps)
    ...
    topic_img = rosbag2_py._storage.TopicMetadata(
            name=topic + topic_ext,
            type='sensor_msg/msg/Image',
            serialization_format='cdr')
    bag.create_topic(topic_img)
    topic_camera = rosbag2_py._storage.TopicMetadata(
            name=topic + '/camera_info',
            type='sensor_msg/msg/CameraInfo',
            serialization_format='cdr')
    bag.create_topic(topic_camera)
    for dt, filename in iterable:
        ...
        header = Header(stamp=Time(sec=dt.seconds, nanosec=dt.microseconds*1000),
                        frame_id=camera_frame_id)
        msg_img = bridge.cv2_to_imgmsg(cv_image, encoding=encoding)
        msg_img.header = header
        msg_camera = calib
        calib.header = header
        bag.write(topic + topic_ext, serialize_message(msg_img), int(dt.total_seconds()*10**6))
        bag.write(topic + '/camera_info', serialize_message(msg_camera),  int(dt.total_seconds()*10**6))
...
def save_static_transforms(bag, transforms, timestamps):
    ...
    for timestamp in timestamps:
        time = rclpy.Time.from_sec(float(timestamp.strftime("%s.%f")))
...
def run_kitti2bag():
    ...
    if args.kitti_type.find("raw") != -1:
        ...
        storage_options = rosbag2_py._storage.StorageOptions(
            uri="kitti_data_raw_{}_sequence_{}.bag".format(args.kitti_type[5:],args.sequence),
            storage_id='sqlite3')
        ...
        bag = rosbag2_py.SequentialWriter()
        storage_options = rosbag2_py._storage.StorageOptions(
            uri="kitti_data_odometry_{}_sequence_{}.bag".format(args.kitti_type[5:],args.sequence),
            storage_id='sqlite3')
        converter_options = rosbag2_py._storage.ConverterOptions('', '')
        bag.open(storage_options, converter_options)
        ...
        current_epoch = (datetime.utcnow() - datetime(1970, 1, 1))
        # Export
        ...
        for camera in used_cameras:
            save_camera_data(bag, args.kitti_type, kitti, util, bridge, camera=camera[0], camera_frame_id='/camera', topic=camera[2], initial_time=current_epoch)
