...
NodeEvaluation::NodeEvaluation(const rclcpp::NodeOptions &options) :
  ...
  _trajOptFileName = declare_parameter("trajOptFilename", "/tmp/trajectory_opt.txt");
  ...
  declare_parameter("gtTrajectoryFile", "/media/data/dataset/rgbd_dataset_freiburg2_desk/rgbd_dataset_freiburg2_desk-groundtruth.txt");

  _pathGt.header.frame_id = "world";
  try {
    _trajGt = vslam::evaluation::tum::loadTrajectory(get_parameter("gtTrajectoryFile").as_string(), true);
    convert(*_trajGt, _pathGt);
  } catch (const std::exception &e) {
    RCLCPP_ERROR(get_logger(), "Error during loading ground truth trajectory: [%s]. Will not compute KPIs or show GT.", e.what());
  }
}
void NodeEvaluation::callbackOdom(const nav_msgs::msg::Odometry::ConstSharedPtr msg) {
  vslam::Pose pose;
  vslam::Timestamp t;
...
void NodeEvaluation::callbackPath(nav_msgs::msg::Path::ConstSharedPtr path) {
  convert(*path, *_trajAlgo);
  vslam::evaluation::tum::writeTrajectory(*_trajAlgo, _algoFileName, false);
  if (_trajGt) {
    try {
      // auto rpe =
      //   std::make_shared<pd::vslam::evaluation::RelativePoseError>(_trajAlgo, _trajGt, 1.0);
    ...
    _pathGt.header.stamp = path->header.stamp;
    _pub->publish(_pathGt);
  }
}
