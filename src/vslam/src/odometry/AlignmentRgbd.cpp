...
Pose AlignmentRgbd::align(
  ...
  const cv::Mat &depth1,
  const SE3d &guess,
  const Mat6d &guessCovariance) const {
...
AlignmentRgbd::Results::UnPtr AlignmentRgbd::align(Frame::ConstShPtr frame0, Frame::ConstShPtr frame1, const Pose &prior) const {
  return align(frame0->features(), frame1, prior);
}
AlignmentRgbd::Results::UnPtr AlignmentRgbd::align(Feature2D::VecConstShPtr features, Frame::ConstShPtr frame1, const Pose &prior) const {
  ...
  for (const auto &level : _levels) {
    ...
    for (int iter = 0; iter < _maxIterations; iter++) {
      ...
      if (constraints[level].size() < 6) {
        reason = format("Not enough constraints: {} at {} iteration {}", constraints[level].size(), level, iter);
        MLOG(WARNING) << reason;
        break;
      }
      {
        ...
        scale[level] = _weightFunction->scale().cast<double>();
      }
...
AlignmentRgbd::setupConstraints(const Frame::VecConstShPtr &framesRef, const Feature2D::VecConstShPtr &features, int level) const {
  ...
  std::transform(exec_policy, features.begin(), features.end(), constraints.begin(), [&](const auto &ft) {
    const Vec2d uv = ft->position() / std::pow(2.0, level);
    auto frame = ft->frame();
    auto cam = ft->frame()->camera(level);
    ...
    c->p0 = Vec3d(frame->p3d(uv(1), uv(0), level)).cast<float>();
    c->iz0 = Vec2f(frame->intensity(uv(1), uv(0), level) / 255., c->p0.z());

    // Dont we have to recompute this each iteration? NO we always linearize and differentiate around the identity warp
    const Mat<float, 1, 2> JI(frame->dIx(uv(1), uv(0), level), frame->dIy(uv(1), uv(0), level));
    const Mat<float, 1, 2> JZ(frame->dZx(uv(1), uv(0), level), frame->dZy(uv(1), uv(0), level));

    c->J.row(0) = 1. / 255. * JI * jacobian::project_p<float>(c->p0, cam->fx(), cam->fy()) * jacobian::transform_se3(SE3f(), c->p0);
    c->JZJw = JZ * jacobian::project_p<float>(c->p0, cam->fx(), cam->fy()) * jacobian::transform_se3(SE3f(), c->p0);
    return c;
  });
...
AlignmentRgbd::Constraint::VecShPtr AlignmentRgbd::computeResidualsAndJacobian(
  const AlignmentRgbd::Constraint::VecShPtr &constraints, Frame::ConstShPtr f1, const std::vector<SE3f> &motion, int level) const {
...
NormalEquations AlignmentRgbd::computeNormalEquations(const std::vector<AlignmentRgbd::Constraint::ShPtr> &constraints, int level) const {
...
NormalEquations AlignmentRgbd::computeNormalEquations(const Pose &prior, const SE3f &pose) const {
