...
void MatchCandidates::drawFeature(
...
FeatureDisplacement::FeatureDisplacement(
  const Frame::VecConstShPtr & frames, const Point3D::VecConstShPtr & points, unsigned int maxWidth,
  unsigned int nRows)
...
FeatureDisplacement::FeatureDisplacement(
  const Frame::VecConstShPtr & frames, unsigned int maxWidth, unsigned int nRows)
    ...
    for (size_t i = 0U; i < _frames.size(); i++) {
      ...
      if (pInF) {
        ...
        if (reprojection.allFinite()) {
          ...
          cv::circle(
            mat, cv::Point(reprojection.x(), reprojection.y()), radius, color, error > 10 ? 3 : 1);
          cv::rectangle(
            mat, cv::Point(pInF->position().x() - radius, pInF->position().y() - radius),
            cv::Point(pInF->position().x() + radius, pInF->position().y() + radius), color,
            error > 10 ? 3 : 1);
