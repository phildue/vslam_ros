...
public:
  ...
  Frame(const cv::Mat &gray, const cv::Mat &depth, Camera::ConstShPtr cam, const Timestamp &t = 0U, const Pose &pose = Pose());

  Frame(const cv::Mat &gray, Camera::ConstShPtr cam, const Timestamp &t = 0U, const Pose &pose = Pose());
  ...
  const float &dZx(int v, int u, size_t level = 0) const { return _dZ.at(level).at<cv::Vec2f>(v, u)[0]; }
  const float &dZy(int v, int u, size_t level = 0) const { return _dZ.at(level).at<cv::Vec2f>(v, u)[1]; }
