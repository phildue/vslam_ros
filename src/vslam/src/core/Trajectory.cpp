...
Trajectory::Trajectory(const std::map<Timestamp, Pose::ShPtr> &poses) :
    _poses(poses) {}
...
Pose::ShPtr Trajectory::poseAt(Timestamp t) {
...
Pose::ConstShPtr Trajectory::interpolateAt(Timestamp t) const {
  ...
  if (it == _poses.begin() || it == _poses.end()) {
    ...
    throw std::runtime_error(format(
      ...
      to_time_point(t),
      to_time_point(_poses.begin()->first),
      to_time_point(_poses.rbegin()->first),
      to_time_point(t - ref)));
  }
...
void Trajectory::append(Timestamp t, const Pose &pose) { _poses[t] = std::make_shared<Pose>(pose); }
...
Trajectory Trajectory::inverse() const {
  std::map<Timestamp, Pose::ShPtr> posesInverted;
  for (auto t_p : _poses) {
