...
void init(int seed) { eng = std::default_random_engine(seed); }

template <typename T = double> T U(T min, T max) {
...
template <typename T = double> T chi2(double dof) {
...
template <typename T = double> T N(T mean, T stddev) {
