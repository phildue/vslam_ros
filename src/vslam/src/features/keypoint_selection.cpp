...
std::vector<Vec2d> selectManual(Frame::ConstShPtr f, int patchSize) {
  ...
  std::vector<cv::Scalar> colors(
    500, cv::Scalar((double)std::rand() / RAND_MAX * 255, (double)std::rand() / RAND_MAX * 255, (double)std::rand() / RAND_MAX * 255));
  ...
  while (cv::waitKey(10) != 27 /*ESC*/) {
    ...
    for (size_t i = 0; i < cc.keypoints.size(); i++) {
      ...
      cv::rectangle(img_, cv::Point(kp.x() - radius, kp.y() - radius), cv::Point(kp.x() + radius, kp.y() + radius), colors[i], 1);
    }
  }
  MLOG(INFO) << format("Selected keypoints: {}", cc.keypoints.size());
  return cc.keypoints;
}
