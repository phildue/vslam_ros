  ...
  _J(Eigen::MatrixXd::Zero(_T.rows() * _T.cols(), w0->nParameters()))
{
  ...
  size_t idx = 0U;
  std::for_each(
    interestPoints.begin(), interestPoints.end(), [&](auto kp)
    {
...
InverseCompositional::InverseCompositional(
  const Image & templ, const Image & image,
  std::shared_ptr<Warp> w0,
  std::shared_ptr<least_squares::Loss> l, double minGradient,
  std::shared_ptr<const least_squares::Prior> prior)
: InverseCompositional(templ, algorithm::gradX(templ).cast<double>(),
    algorithm::gradY(templ).cast<double>(), image, w0, l, minGradient, prior) {}
...
least_squares::NormalEquations::ConstShPtr InverseCompositional::computeNormalEquations()
{
  ...
  std::for_each(
    _interestPoints.begin(), _interestPoints.end(),
    [&](auto kp) {
      Eigen::Vector2d uvI = _w->apply(kp.pos.x(), kp.pos.y());
      const bool visible = 1 < uvI.x() && uvI.x() < _I.cols() - 1 && 1 < uvI.y() && uvI.y() < _I.rows() - 1 && std::isfinite(
        uvI.x());
      if (visible) {
        ...
        R(
          kp.pos.y(),
          kp.pos.x()) = (double)IWxp(kp.pos.y(), kp.pos.x()) - (double)_T(kp.pos.y(), kp.pos.x());
