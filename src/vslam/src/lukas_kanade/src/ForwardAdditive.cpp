...
least_squares::NormalEquations::ConstShPtr ForwardAdditive::computeNormalEquations()
{
  ...
  std::for_each(
    _interestPoints.begin(), _interestPoints.end(), [&](auto kp) {
      ...
      1 < uvWarped.y() && uvWarped.y() < _Iref.rows() - 1)
      {
        ...
        J.row(
          kp.y() * _Iref.cols() +
          kp.x()) = (dIxWp(kp.y(), kp.x()) * Jwarp.row(0) + dIyWp(kp.y(), kp.x()) * Jwarp.row(1));
  ...
  auto it = std::copy_if(
    std::execution::par_unseq, _interestPoints.begin(),
    _interestPoints.end(), interestPointsVisible.begin(),
    [&](auto kp) {
      Eigen::Vector2d uvI = _w->apply(kp.x(), kp.y());
      const bool visible = 1 < uvI.x() && uvI.x() < _Iref.cols() - 1 && 1 < uvI.y() && uvI.y() < _Iref.rows() - 1 && std::isfinite(
        uvI.x());
      if (visible) {
  ...
  std::for_each(
    ...
    [&](auto kp)
    {
      ...
      if (!std::isfinite(J.row(kp.y() * _T.cols() + kp.x()).norm()) ||
      !std::isfinite(R(kp.y(), kp.x())) || !std::isfinite(W(kp.y(), kp.x())))
      {
        std::stringstream ss;
        ss << "NaN during LK with: R = " << R(kp.y(), kp.x()) << " W = " << W(
          kp.y(),
          kp.x()) << " J = " << J.row(kp.y() * _T.cols() + kp.x()) << " at: " << kp.transpose();
